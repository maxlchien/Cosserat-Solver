...
contains
  ...
  subroutine greens_displacement_force_points_vectorized_wrapper(points, n_points, omega, n_omega, &
    rho, lam, mu, force_use_openmp, force_no_openmp, result_real, result_imag) &
    bind(C, name="greens_displacement_force_points_vectorized_wrapper")
    integer(c_int), intent(in), value :: n_points, n_omega
    real(c_double), intent(in) :: points(3, n_points)
    real(c_double), intent(in) :: omega(n_omega)
    real(c_double), intent(in), value :: rho, lam, mu
    logical(c_bool), intent(in), value :: force_use_openmp, force_no_openmp
    real(c_double), intent(out) :: result_real(n_omega, 3, 3, n_points)
    real(c_double), intent(out) :: result_imag(n_omega, 3, 3, n_points)

    real(rk), allocatable :: omega_q(:)
    real(rk), allocatable :: points_q(:,:)
    complex(rk), allocatable :: result_quad(:,:,:,:)
    real(rk) :: rho_q, lam_q, mu_q
    logical :: force_use, force_no
    ...
    ! Allocate arrays
    allocate(omega_q(n_omega))
    allocate(points_q(3, n_points))
    allocate(result_quad(n_omega, 3, 3, n_points))

    ! Convert inputs to rk kind
    points_q = real(points, rk)
    rho_q = real(rho, rk)
    lam_q = real(lam, rk)
    mu_q = real(mu, rk)
    force_use = force_use_openmp
    ...
    ! Call batched Fortran function with conditional optional arguments
    if (force_use) then
      result_quad = greens_displacement_force_points_vectorized(points_q, n_points, omega_q, n_omega, &
                                      rho_q, lam_q, mu_q, force_use_openmp=.true.)
    else if (force_no) then
      result_quad = greens_displacement_force_points_vectorized(points_q, n_points, omega_q, n_omega, &
                                      rho_q, lam_q, mu_q, force_no_openmp=.true.)
    ...
    deallocate(points_q)
    deallocate(result_quad)
  end subroutine greens_displacement_force_points_vectorized_wrapper
