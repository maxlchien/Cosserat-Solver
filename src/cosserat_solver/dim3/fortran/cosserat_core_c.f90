...
contains
  ...
  subroutine greens_mixed_force_points_vectorized_wrapper(points, n_points, omega, n_omega, &
    rho, lam, mu, nu, J, lam_c, mu_c, nu_c, &
    force_use_openmp, force_no_openmp, result_real, result_imag) &
    bind(C, name="greens_mixed_force_points_vectorized_wrapper")
    integer(c_int), intent(in), value :: n_points, n_omega
    real(c_double), intent(in) :: points(3, n_points)
    real(c_double), intent(in) :: omega(n_omega)
    real(c_double), intent(in), value :: rho, lam, mu, nu, J, lam_c, mu_c, nu_c
    logical(c_bool), intent(in), value :: force_use_openmp, force_no_openmp
    real(c_double), intent(out) :: result_real(n_omega, 6, 6, n_points)
    real(c_double), intent(out) :: result_imag(n_omega, 6, 6, n_points)
    ...
    allocate(result_quad(n_omega, 6, 6, n_points))
    ...
    if (force_use) then
      result_quad = greens_mixed_force_points_vectorized(points_q, n_points, omega_q, n_omega, &
                                      rho_q, lam_q, mu_q, nu_q, J_q, lam_c_q, mu_c_q, nu_c_q, force_use_openmp=.true.)
    else if (force_no) then
      result_quad = greens_mixed_force_points_vectorized(points_q, n_points, omega_q, n_omega, &
                                      rho_q, lam_q, mu_q, nu_q, J_q, lam_c_q, mu_c_q, nu_c_q, force_no_openmp=.true.)
    ...
    deallocate(points_q)
    deallocate(result_quad)
  end subroutine greens_mixed_force_points_vectorized_wrapper
